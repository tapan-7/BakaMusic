...
import { ScannedTrack, extractTrackMetadata } from '../services/musicScannerService';
...
export const TrackListItem: React.FC<TrackListItemProps> = ({ track, onPress, layout = 'row' }) => {
  const currentTrack = usePlayerStore(state => state.currentTrack);
  const isPlaying = currentTrack?.id === track.id;
  
  // Local state for lazy metadata
  const [localArtwork, setLocalArtwork] = useState<string | undefined>(track.artwork);
  const [localArtist, setLocalArtist] = useState<string>(track.artist || 'Unknown Artist');
  ...
  if (layout === 'column') {
    return (
      ...
      >
        ...
        <Text numberOfLines={1} className={`font-medium text-sm ${isPlaying ? 'text-primary font-bold' : 'text-white'}`}>
          {localTitle}
        </Text>
  ...
  return (
    ...
    >
      ...
      <View className="flex-1 justify-center">
        <Text 
          numberOfLines={1} 
          className={`font-medium text-base mb-1 ${isPlaying ? 'text-primary font-bold' : 'text-white'}`}
        >
          {localTitle}
        </Text>
